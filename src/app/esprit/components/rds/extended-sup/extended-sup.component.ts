...
@Component({
    selector: 'app-extended-sup',
    templateUrl: './extended-sup.component.html',
    styleUrls: ['./extended-sup.component.scss'],
...
export class ExtendedSupComponent implements OnInit, AfterViewInit {
    anomalies: Anomaly[] = [];
    loading = false;
    errorMessage = '';
    ...
    // Bar Chart for db_instance_identifier with engine and engine_version
    rdsDetailChartType: ChartType = 'bar';
    rdsDetailChartOptions: ChartOptions<'bar'> = {
        scales: {
            y: {
                beginAtZero: true,
                max: 5,
        ...
        plugins: {
            ...
            title: { display: true, text: 'RDS Instances with Engine Details' },
            tooltip: {
                callbacks: {
                    label: (tooltipItem) => {
    ...
    loadAnomalies(): void {
        this.loading = true;
        this.anomalyService.getAnomalies('AWS RDS Extended Support Version').subscribe({
